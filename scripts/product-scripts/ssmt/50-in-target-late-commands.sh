...
filter_available_packages() {
  ...
  echo "[ssmt-in-target] Available packages: ${#AVAILABLE_PKGS[@]}"
  if [ "$before_count" -ne "${#AVAILABLE_PKGS[@]}" ]; then
    echo "[ssmt-in-target] Pruned duplicate package entries: $((before_count - ${#AVAILABLE_PKGS[@]}))"
  fi
  if [ "${#MISSING_PKGS[@]}" -gt 0 ]; then
    echo "[ssmt-in-target] Skipping missing packages (${#MISSING_PKGS[@]}): ${MISSING_PKGS[*]}"
  fi
}
...
install_packages_one_by_one() {
  ...
  [ "$total" -gt 0 ] || {
    echo "[ssmt-in-target] No installable packages found in current repositories"
    return 0
  }

  echo "[ssmt-in-target] Installing $total packages from REQUIRED_DEBS (one at a time)"
  for pkg in "${AVAILABLE_PKGS[@]}"; do
    i=$((i + 1))
    apt_candidate="$(apt-cache policy "$pkg" 2>/dev/null | awk '/Candidate:/ {print $2; exit}')"
    if [ -z "$apt_candidate" ] || [ "$apt_candidate" = "(none)" ]; then
      echo "[ssmt-in-target] Skipping $pkg (no installation candidate)"
      continue
    fi

    echo "[ssmt-in-target] [$i/$total] Installing $pkg"
    if command -v timeout >/dev/null 2>&1; then
      if ! timeout "$INSTALL_TIMEOUT_SECS" apt-get "${APT_OPTS[@]}" install -y --no-install-recommends "$pkg"; then
        echo "[ssmt-in-target] Skipping $pkg (install failed or timed out, continuing)"
      fi
      continue
    fi

    if ! apt-get "${APT_OPTS[@]}" install -y --no-install-recommends "$pkg"; then
      echo "[ssmt-in-target] Skipping $pkg (install failed, continuing)"
    fi
  done
}
...
finalize_package_state() {
  ...
  if [ -z "$audit_out" ]; then
    echo "[ssmt-in-target] No broken package state detected; skipping fix-broken step"
    return 0
  fi

  echo "[ssmt-in-target] Broken package state detected; running fix-broken install"
  if command -v timeout >/dev/null 2>&1; then
    if ! timeout 600 apt-get "${APT_OPTS[@]}" -y -f install; then
      echo "[ssmt-in-target] fix-broken install failed or timed out; continuing"
    fi
    return 0
  fi

  if ! apt-get "${APT_OPTS[@]}" -y -f install; then
    echo "[ssmt-in-target] fix-broken install failed; continuing"
  fi
}
...
main() {
  ...
  echo "**********************************************************************"
  echo "Running ssmt/50-in-target-late-commands.sh"

  if ! apt-get "${APT_OPTS[@]}" update; then
    echo "[ssmt-in-target] Repository update failed. Skipping required package installation."
    return 0
  fi
